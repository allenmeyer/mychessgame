...
function isLegalMove(piece, candidate_location) {
	var team = piece.team;
	var oldLoc = piece.location;
	var wouldCapture = false;
	var capturedPieceTeamIndex;
	if (team == 'white') {
		if (getPieceByLocation(candidate_location) !== undefined) { 
			if (getPieceByLocation(candidate_location).team == 'black') { // There is a black piece here
				capturedPieceTeamIndex = getPieceByLocation(candidate_location).teamindex;
				blackpieces[getPieceByLocation(candidate_location).teamindex].isCaptured = true;
				whitepieces[piece.teamindex].location = candidate_location;
				wouldCapture = true;
			}
		} else {
			whitepieces[piece.teamindex].location = candidate_location;
		...
		if (getPieceByLocation(candidate_location) !== undefined) { 
			if (getPieceByLocation(candidate_location).team == 'white') { // There is a white piece here
				capturedPieceTeamIndex = getPieceByLocation(candidate_location).teamindex;
				whitepieces[getPieceByLocation(candidate_location).teamindex].isCaptured = true;
				blackpieces[piece.teamindex].location = candidate_location;
				wouldCapture = true;
			}
		} else {
			blackpieces[piece.teamindex].location = candidate_location;
...
function movePiece(dot) {
	...
	clearDots();
	if (!promotion) {
		recordMove(prevMove, castledinfo, undefined, false);
	}
	paused = false;
	started = true;
}
...
function recordMove(move, castled, promotion_type, promotion_flag) {
	...
	if (castled == 'queenside') {
		...
		text += '=' + getPromotionNameByType(promotion_type);
...
function promotePiece() {
	...
	recordMove(prevMove, '', promotionType, true);
...
function knightMoveableSquares(knight) {
	if (!knight.isCaptured) {
		...
		if (!isOutOfBounds(square8loc)) {
			if (isLegalMove(knight, square8loc)) {
